...
cc.Class({
  ...
  revealScene() {
    ...
    cc.tween(this.maskLayer).to(0.3, { opacity: 0 }).start();
  },
  restart() {
    this.maskLayer.color = cc.Color.BLACK;
  ...
  gameOver() {
    ...
    this.bg.getComponent("scrollerbg").stopScroll();
    // 停止游戏输入监听
  ...
  showGameOverMenu() {
    ...
    let showMedal = (err, spriteFrame) => {
      if (this.score >= this.goldScore) {
        medalNode.getComponent(cc.Sprite).spriteFrame =
          spriteFrame._spriteFrames.medal_gold;
      } else if (this.score >= this.silverScore) {
        medalNode.getComponent(cc.Sprite).spriteFrame =
          spriteFrame._spriteFrames.medal_silver;
      } else {
    ...
    cc.tween(gameOverNode)
      ...
      .start();
    cc.tween(startButtonNode).delay(0.3).to(0.5, { opacity: 255 }).start();
    cc.tween(resultBoardNode)
      .delay(0.3)
      .to(
        0.9,
        { position: cc.v2(resultBoardNode.x, 200) },
        { easing: "cubicInOut" }
      )
      .start();
  },
